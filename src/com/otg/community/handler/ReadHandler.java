...
import com.otg.community.activities.LardLordInsertActivity;
import com.otg.community.domain.IdentityCard;
import com.otg.community.utils.MessageUtils;
import com.lz.nfc.jni.NFCJni;
import com.otg.community.utils.Server;
import com.otg.community.utils.UncodeUtils;
import com.otg.community.utils.ByteHex;
...
public class ReadHandler {
    ...
    */
    private void parseReadIDCard(byte[] bytes) throws Exception {
        ...
        message.what = LardLordInsertActivity.READ_IDCARD_SUCCESS;
            ...
            public void run() {
                if (cfd > 0) {
                    ...
                    if(remote_ip!=null&&remote_ip.length()>0&&remote_port!=null&&remote_port.length()>0) {
                        ...
                        Message message = new Message();
                        message.what = LardLordInsertActivity.PLASE_INIT_SERVER;
                        message.obj = "请先初始化服务器配置";
                        handler.sendMessage(message);
                    }
                }else {
                    Message message = new Message();
                    message.what = LardLordInsertActivity.NOT_FOUND_DEVICE;
                    message.obj = "未找到读卡设备";
                    handler.sendMessage(message);
                }
            }
